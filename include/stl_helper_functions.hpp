...
template<typename T,
  typename = std::enable_if_t<is_char_pointer_type_v<T> || is_valid_string_view_type_v<T> || is_valid_char_type_v<T>>>
  ...
  Args &&... args)
{
  ...
  do {
    ...
    const int number_of_chars_written =
      swprintf(&output_buffer[0], output_buffer.size(), format_string, std::forward<Args>(args)...);
...
StringType find_longest_word(const StringType &src, ContainerType &haystack)
{
  ...
  if constexpr (!has_find_member_function_v<ContainerType, T> && ((has_sort_member_function_v<ContainerType> && is_operator_less_than_defined_v<U>) || is_operator_less_than_defined_v<U>)) {
  ...
  const std::locale &loc = std::locale{})
{
  ...
  if (std::has_facet<std::ctype<char_type>>(loc)) {
    const auto &f = std::use_facet<std::ctype<char_type>>(loc);
    std::transform(std::cbegin(src_uc), std::cend(src_uc), std::begin(src_uc), [&f](const auto ch) { return f.toupper(ch); });

  } else {
    std::transform(
      std::cbegin(src_uc), std::cend(src_uc), std::begin(src_uc), [](const auto ch) { return static_cast<char_type>(std::toupper(ch)); });
  }
...
template<
  ...
  typename = std::enable_if_t<
    is_valid_char_type_v<
      typename std::iterator_traits<IteratorType>::value_type> && is_all_of_v<typename std::iterator_traits<IteratorType>::value_type, get_char_type_t<NeedleType>, get_char_type_t<NeedleSeparatorType>>>>
std::vector<
  ...
  typename = std::enable_if_t<
    std::is_assignable_v<
      ...
      std::add_rvalue_reference_t<
        typename std::iterator_traits<ForwardIterType1>::value_type>> || std::is_assignable_v<std::add_lvalue_reference_t<typename std::iterator_traits<ForwardIterType2>::value_type>, std::add_lvalue_reference_t<const typename std::iterator_traits<ForwardIterType1>::value_type>>>>
constexpr std::pair<ForwardIterType1, ForwardIterType2> move_while(
  ...
  Predicate p) noexcept((std::
                           is_nothrow_move_assignable_v<
                             ...
                             std::add_lvalue_reference_t<
                               const typename std::iterator_traits<
                                 ForwardIterType1>::value_type>> || std::is_nothrow_assignable_v<std::add_lvalue_reference_t<typename std::iterator_traits<ForwardIterType2>::value_type>, std::add_lvalue_reference_t<const typename std::iterator_traits<ForwardIterType1>::value_type>>)&&std::
