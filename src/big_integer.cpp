  ...
  const big_integer &rhs)
{
  if (!lhs.is_nan() && !rhs.is_nan()) {
    ...
    const bool is_division_negative{
      (!lhs.is_negative_number() && rhs.is_negative_number()) || (lhs.is_negative_number() && !rhs.is_negative_number())
    };
  ...
  const big_integer &rhs)
{
  if (!lhs.is_nan() && !rhs.is_nan()) {
    ...
    if (lhs.abs().is_positive_one()) {
      ...
      return !is_negative_number ? big_integer::positive_one : big_integer::negative_one;
...
big_integer &big_integer::operator++()
{
  if (!this->is_nan()) {
    ...
    big_integer temp{ this->add_two_big_integers_together(big_integer::positive_one) };
    this->swap(temp);
  }
...
big_integer big_integer::operator++(int)
{
  ...
  big_integer temp_bi{ this->add_two_big_integers_together(big_integer::positive_one) };
  this->swap(temp_bi);
...
big_integer &big_integer::operator--()
{
  ...
  if (!this->is_nan()) {
    ...
    big_integer temp{ this->add_two_big_integers_together(big_integer::negative_one) };
    this->swap(temp);
  }
...
big_integer big_integer::operator--(int)
{
  ...
  big_integer temp_bi{ this->add_two_big_integers_together(big_integer::negative_one) };
  this->swap(temp_bi);
  ...
  const big_integer &rhs) const
{
  ...
  if ((this->is_negative_number() && !rhs.is_negative_number()) || (!this->is_negative_number() && rhs.is_negative_number())) {
    ...
    is_negative_addition =
      (this->is_negative_number() && is_abs_first_greater_abs_second) || (rhs.is_negative_number() && is_abs_second_greater_than_abs_first);
  ...
  const big_integer &rhs) const
{
  ...
  for (long i{ static_cast<long>(rhs.decimal_digits_.size()) - 1 }; i >= 0; --i) {
    ...
    for (long j{ static_cast<long>(decimal_digits_.size()) - 1 }; j >= 0; --j) {
      int product =
        std::abs(decimal_digits_[j]) * std::abs(rhs.decimal_digits_[i]) + carry;
      carry = product / 10;
      product %= 10;
  ...
  if (base_ != number_base::decimal) {
    ...
    for (size_t i{}; i < binary_digits_.size(); ++i) {
      for (long j = static_cast<long>(bcd_bin_buffer_size) - 1; j >= 3;
           j -= 4) {
        unsigned value = (bcd_digits[j] ? 1 : 0) + (bcd_digits[j - 1] ? 2 : 0) + (bcd_digits[j - 2] ? 4 : 0) + (bcd_digits[j - 3] ? 8 : 0);
