...
const std::string& big_integer::get_decimal_number() const noexcept {
...
std::string big_integer::get_binary_number(
  ...
  size_t bin_str_len{binary_number_str.length()};
  const size_t sign_offset{is_negative_number_ ? 1U : 0U};

  if (!prefix.empty() && bin_str_len >= prefix_len &&
      !stl::helper::str_contains(binary_number_str, prefix, false)) {
    const size_t first_digit_pos{binary_number_str.find_first_of("01")};

    binary_number_str.replace(std::cbegin(binary_number_str) + sign_offset,
                              std::cbegin(binary_number_str) + first_digit_pos,
                              prefix);
    bin_str_len = binary_number_str.length();
  }

  if (!postfix.empty() && bin_str_len >= postfix_len &&
      !stl::helper::str_ends_with(binary_number_str, postfix, false)) {
  ...
  if (decimal_digits_.size() == 1U && decimal_digits_[0] == -1) {
    ...
    if (carry != 0) {
      ...
      decimal_number_str_.insert(std::cbegin(decimal_number_str_) + first_,
                                 '1');
    } else if (zero_pos != std::string::npos &&
               decimal_number_str_.find_first_of("123456789") > zero_pos) {
  ...
  if (result.decimal_digits_.size() == 1U && result.decimal_digits_[0] == -1) {
    ...
    if (carry != 0) {
      ...
      result.decimal_number_str_.erase(
          ...
          std::cbegin(result.decimal_number_str_) + result.first_ + zero_pos +
              1);
    }
  }
...
big_integer& big_integer::operator--() {
  if (decimal_digits_[0] == 0) {
    ...
    if (zero_pos != std::string::npos &&
        decimal_number_str_.find_first_of("123456789", first_) > zero_pos) {
      decimal_digits_.erase(std::cbegin(decimal_digits_),
                            std::cbegin(decimal_digits_) + zero_pos + 1);
      decimal_number_str_.erase(
          std::cbegin(decimal_number_str_) + first_,
...
big_integer big_integer::operator--(int) const {
  ...
  if (result.decimal_digits_[0] == 0) {
    ...
    for (long i = static_cast<long>(result.decimal_digits_.size()) - 1;
      ...
      } else if (result.is_negative_number_ &&
                 -9 == result.decimal_digits_[i]) {
        result.decimal_digits_[i] = 0;
    ...
    if (zero_pos != std::string::npos &&
        result.decimal_number_str_.find_first_of("123456789", first_) >
            zero_pos) {
      result.decimal_digits_.erase(
          std::cbegin(result.decimal_digits_),
          std::cbegin(result.decimal_digits_) + zero_pos + 1);
...
big_integer big_integer::add_two_decimal_numbers_together(
  ...
  if (this->is_zero())
    return big_integer{rhs};

  if (rhs.is_zero())
    return big_integer{*this};

  std::vector<int> result{};
  result.reserve(std::max(decimal_digits_.size(), rhs.decimal_digits_.size()) +
                 1);
  ...
  const auto last_non_zero_digit_pos =
      std::find_if(std::crbegin(result), std::crend(result),
...
big_integer big_integer::multiply_two_decimal_numbers(
  ...
  for (long i{static_cast<long>(rhs.decimal_digits_.size()) - 1}; i >= 0; --i) {
    ...
    for (long j{static_cast<long>(decimal_digits_.size()) - 1}; j >= 0; --j) {
      byte product{static_cast<byte>(std::abs(decimal_digits_[j]) *
                                         std::abs(rhs.decimal_digits_[i]) +
                                     carry)};
      carry = product / 10;
      product %= 10;
...
std::string big_integer::get_hexadecimal_number(
  ...
  size_t hex_str_len{hex_number_str.length()};
  const size_t sign_offset{is_negative_number_ ? 1U : 0U};

  if (!prefix.empty() && hex_str_len >= prefix_len &&
      !stl::helper::str_contains(hex_number_str, prefix, false)) {
    const size_t first_digit_pos{
        hex_number_str.find_first_of("0123456789abcdefABCDEF")};

    hex_number_str.replace(std::cbegin(hex_number_str) + sign_offset,
                           std::cbegin(hex_number_str) + first_digit_pos,
                           prefix);
    hex_str_len = hex_number_str.length();
  }

  if (!postfix.empty() && hex_str_len >= postfix_len &&
...
std::string big_integer::get_octal_number(const std::string& prefix,
  ...
  size_t octal_str_len{octal_number_str.length()};
  const size_t sign_offset{is_negative_number_ ? 1U : 0U};

  if (!prefix.empty() && octal_str_len >= prefix_len &&
      !stl::helper::str_contains(octal_number_str, prefix, false)) {
    const size_t first_digit_pos{octal_number_str.find_first_of("01234567")};

    octal_number_str.replace(std::cbegin(octal_number_str) + sign_offset,
                             std::cbegin(octal_number_str) + first_digit_pos,
                             prefix);
    octal_str_len = octal_number_str.length();
  }

  if (!postfix.empty() && octal_str_len >= postfix_len &&
      !stl::helper::str_ends_with(octal_number_str, postfix, false)) {
...
void big_integer::convert_number_to_decimal_and_store() {
  if (base_ != number_base::decimal) {
    if (binary_digits_.empty()) {
      const byte number_of_bits_for_digit{
          number_base_number_of_bits_.at(base_)};
      ...
      std::for_each(
          ...
          std::cbegin(decimal_number_str_) + last_, [&](const auto ch) {
            ...
            for (byte i{}; i < number_of_bits_for_digit; ++i) {
              const byte bit_mask{
                  static_cast<byte>(1_b << (number_of_bits_for_digit - 1 - i))};
    ...
    for (size_t i{}; i < binary_digits_.size(); ++i) {
      for (long j = static_cast<long>(bcd_bin_buffer_size) - 1; j >= 3;
           j -= 4) {
        byte value =
            (bcd_digits[j] ? 1_b : 0_b) + (bcd_digits[j - 1] ? 2_b : 0_b) +
            (bcd_digits[j - 2] ? 4_b : 0_b) + (bcd_digits[j - 3] ? 8_b : 0_b);
