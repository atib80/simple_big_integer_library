...
const std::unordered_map<number_base, const char *> big_integer::number_base_prefix_{ { number_base::binary, "0b" },
...
big_integer org::atib::numerics::operator%(const big_integer &lhs, const big_integer &rhs)
{
  if (!lhs.is_nan() && !rhs.is_nan()) {
#ifndef _BIG_INTEGER_LIBRARY_NO_THROW_
    if (rhs.is_zero()) {
...
big_integer org::atib::numerics::operator&(const big_integer &lhs, const big_integer &rhs)
{
  if (!lhs.is_nan() && !rhs.is_nan()) {
    ...
    if (lhs.is_zero()) return big_integer{};

    if (rhs.is_zero()) return big_integer{};
...
std::ostream &org::atib::numerics::operator<<(std::ostream &os, const big_integer &bi)
{
  return (os << bi.get_decimal_number());
}

std::istream &org::atib::numerics::operator>>(std::istream &is, big_integer &bi)
{
  std::string line;
  is >> line;
  bi.reset(line);
  ...
  const size_t number_of_digits) const
{
  ...
  if (!prefix.empty() && !stl::helper::str_contains(decimal_number_str, prefix)) {
    const size_t first_digit_pos{ decimal_number_str.find_first_of("0123456789", sign_offset) };

    decimal_number_str.replace(std::cbegin(decimal_number_str) + sign_offset,
  ...
  const size_t count_of_decimal_digits{ decimal_number_str.length() - (prefix_len + postfix_len + sign_offset) };
  ...
  const size_t number_of_digits) const
{
  ...
  if (!prefix.empty() && !stl::helper::str_contains(binary_number_str, prefix)) {
    const size_t first_digit_pos{ binary_number_str.find_first_of("01", sign_offset) };
  ...
  const size_t number_of_digits) const
{
  ...
  if (!prefix.empty() && !stl::helper::str_contains(octal_number_str, prefix, sign_offset)) {
    const size_t first_digit_pos{ octal_number_str.find_first_of("01234567", sign_offset) };
  ...
  if (base_ != number_base::decimal) {
    ...
    for (size_t i{}; i < binary_digits_.size(); ++i) {
      for (long j = static_cast<long>(bcd_bin_buffer_size) - 1; j >= 3;
           j -= 4) {
        unsigned value =
          (bcd_digits[j] ? 1 : 0) + (bcd_digits[j - 1] ? 2 : 0) + (bcd_digits[j - 2] ? 4 : 0) + (bcd_digits[j - 3] ? 8 : 0);
