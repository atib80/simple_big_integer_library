...
const big_integer big_integer::positive_one{"1"};
const big_integer big_integer::negative_one{"-1"};
...
big_integer org::atib::numerics::operator/(const big_integer &lhs,
    ...
    if (!lhs.is_nan() && !rhs.is_nan()) {
        ...
        while (dividend >= divisor) {
            ...
            --shift_count;
            big_integer big_one{big_integer::positive_one};
...
big_integer org::atib::numerics::operator|(const big_integer &lhs,
    ...
    if (!lhs.is_nan() && !rhs.is_nan()) {
        ...
        const bool is_lhs_longer{lhs_binary_digits.size() >=
                                 rhs_binary_digits.size()};
        ...
        for (size_t i{}; i < shorter_binary_number.size(); ++i) {
            result[longer_binary_number.size() - shorter_binary_number.size() + i] =
                    result[longer_binary_number.size() - shorter_binary_number.size() +
                           i] ||
                    shorter_binary_number[i];
        }
...
big_integer org::atib::numerics::operator&(const big_integer &lhs,
    ...
    if (!lhs.is_nan() && !rhs.is_nan()) {
        ...
        if (lhs.abs().is_positive_one()) {
            const bool is_negative_number{rhs.is_negative_number()};
            return !is_negative_number ? big_integer{"1"} : big_integer{"-1"};

        } else if (rhs.abs().is_positive_one()) {
            const bool is_negative_number{lhs.is_negative_number()};
            return !is_negative_number ? big_integer{"1"} : big_integer{"-1"};
        }
...
std::string big_integer::get_decimal_number(
    ...
    if (!prefix.empty() &&
        !stl::helper::str_contains(decimal_number_str, prefix)) {
        ...
        decimal_number_str.replace(
                ...
                std::cbegin(decimal_number_str) + first_digit_pos, prefix);
    }

    if (!postfix.empty() &&
        !stl::helper::str_ends_with(decimal_number_str, postfix)) {
        const size_t last_digit_pos{decimal_number_str.find_last_of("0123456789")};

        decimal_number_str.replace(
                std::cbegin(decimal_number_str) + last_digit_pos + 1,
                std::cend(decimal_number_str), postfix);
    }
    ...
    if (number_of_digits != 0 && number_of_digits > count_of_decimal_digits)
        decimal_number_str.insert(
                ...
                number_of_digits - count_of_decimal_digits, '0');
...
std::string big_integer::get_binary_number(
    ...
    if (!prefix.empty() &&
        !stl::helper::str_contains(binary_number_str, prefix)) {
        const size_t first_digit_pos{
    ...
    if (!postfix.empty() &&
        !stl::helper::str_ends_with(binary_number_str, postfix)) {
        ...
        binary_number_str.replace(
                ...
                std::cend(binary_number_str), postfix);
    }

    const size_t count_of_bin_digits{binary_number_str.length() -
                                     (prefix_len + postfix_len + sign_offset)};
    ...
    if (number_of_digits != 0 && number_of_digits > count_of_bin_digits)
        binary_number_str.insert(
                ...
                number_of_digits - count_of_bin_digits, '0');
...
big_integer &big_integer::operator++() {
    ...
    big_integer temp{this->add_two_decimal_numbers_together(big_integer::positive_one)};
    this->swap(temp);
...
big_integer big_integer::operator++(int) const {
    ...
    return this->add_two_decimal_numbers_together(big_integer::positive_one);
}

big_integer &big_integer::operator--() {

    big_integer temp{this->add_two_decimal_numbers_together(big_integer::negative_one)};
    this->swap(temp);
...
big_integer big_integer::operator--(int) const {
    ...
    return this->add_two_decimal_numbers_together(big_integer::negative_one);
}

big_integer big_integer::operator<<(const size_t count) const {
    std::vector<bool> shifted_binary_digits{this->binary_digits_};
    if (count != 0U)
        shifted_binary_digits.insert(std::cend(shifted_binary_digits), count,
                                     false);
...
big_integer big_integer::add_two_decimal_numbers_together(
    ...
    if ((this->is_negative_number() && !rhs.is_negative_number()) ||
        ...
        int n = static_cast<int>(greater_number_decimal_digits.size()) - 1;
        int m = static_cast<int>(smaller_number_decimal_digits.size()) - 1;
        int carry{};
        ...
        while (n >= 0 || m >= 0 || carry != 0) {
            ...
            if (n >= 0) {
                if (sum + greater_number_decimal_digits[n] < 0) {
                    sum += 10 + greater_number_decimal_digits[n];
                    --carry;
                } else {
                    sum += greater_number_decimal_digits[n];
                }
                --n;
            }
            if (m >= 0) {

                if (sum + smaller_number_decimal_digits[m] < 0) {
                    sum += 10 + smaller_number_decimal_digits[m];
                    --carry;
                } else {
                    sum += smaller_number_decimal_digits[m];
                }
                --m;
            }
        ...
        int n = static_cast<int>(lhs_digits.size()) - 1;
        int m = static_cast<int>(rhs_digits.size()) - 1;
        int carry{};
    ...
    const auto first_non_zero_digit_pos =
            std::find_if(std::cbegin(result), std::cend(result),
...
big_integer big_integer::multiply_two_decimal_numbers(
    ...
    const bool is_negative_product{
            (is_negative_number_ && !rhs.is_negative_number_) ||
            (!is_negative_number_ && rhs.is_negative_number_)};
    ...
    for (long i{static_cast<long>(rhs.decimal_digits_.size()) - 1}; i >= 0; --i) {
        ...
        for (long j{static_cast<long>(decimal_digits_.size()) - 1}; j >= 0; --j) {
            int product =
                    std::abs(decimal_digits_[j]) * std::abs(rhs.decimal_digits_[i]) +
                    carry;
            carry = product / 10;
            product %= 10;
...
std::string big_integer::get_hexadecimal_number(
    ...
    if (!prefix.empty() &&
        !stl::helper::str_contains(hex_number_str, prefix, sign_offset)) {
        const size_t first_digit_pos{
    ...
    if (!postfix.empty() &&
        !stl::helper::str_ends_with(hex_number_str, postfix, false)) {
        ...
        hex_number_str.replace(std::cbegin(hex_number_str) + last_digit_pos + 1,
                               std::cend(hex_number_str), postfix);
    }

    const size_t count_of_hex_digits{hex_number_str.length() -
                                     (prefix_len + postfix_len + sign_offset)};
    ...
    if (number_of_digits != 0 && number_of_digits > count_of_hex_digits)
        hex_number_str.insert(
                ...
                number_of_digits - count_of_hex_digits, '0');
...
std::string big_integer::get_octal_number(const std::string &prefix,
    ...
    if (!prefix.empty() &&
        !stl::helper::str_contains(octal_number_str, prefix, sign_offset)) {
        const size_t first_digit_pos{
    ...
    if (!postfix.empty() &&
        !stl::helper::str_ends_with(octal_number_str, postfix)) {
        ...
        octal_number_str.replace(std::cbegin(octal_number_str) + last_digit_pos + 1,
                                 std::cend(octal_number_str), postfix);
    }

    const size_t count_of_octal_digits{octal_number_str.length() -
                                       (prefix_len + postfix_len + sign_offset)};
    ...
    if (number_of_digits != 0 && number_of_digits > count_of_octal_digits)
        octal_number_str.insert(
                ...
                number_of_digits - count_of_octal_digits, '0');
...
void big_integer::check_and_process_binary_number_digits(
    ...
    if (binary_number.empty() ||
        (1U == binary_number.size() && 0 == binary_number.front())) {
...
bool big_integer::check_and_process_number_input_digits(
    ...
    number_str.reserve(number.size() + (is_negative_number ? 1 : 0) +
                       (base_ != number_base::decimal ? 2 : 0));
...
bool big_integer::check_and_process_number_input_characters(
    ...
    while (first < last &&
           (('0' == number[first]) || (0 != isspace(number[first])))) {
        ++first;
    }
    ...
    decimal_number_str_.reserve(last - first + (is_negative_number ? 1 : 0) +
                                (base != number_base::decimal ? 2 : 0));
    ...
    std::for_each(std::cbegin(number) + first, std::cbegin(number) + last,
                  [&](const char digit) {
                      if (digit != '_' && digit != '\'')
...
void big_integer::find_2s_complement_for_binary_number() {
    ...
    std::transform(std::cbegin(binary_digits_), std::cend(binary_digits_),
                   std::back_inserter(binary_digits),
                   [](const bool bin_digit) { return !bin_digit; });

    bool carry_bit{true};

    for (int i = binary_digits.size() - 1; i >= 0; --i) {
        const bool result{(carry_bit && !binary_digits.at(i)) ||
                          (!carry_bit && binary_digits.at(i))};
...
void big_integer::convert_number_to_decimal_and_store() {
    if (base_ != number_base::decimal) {
        ...
        std::for_each(std::cbegin(decimal_number_str_) + first_,
                      std::cbegin(decimal_number_str_) + last_, [&](const auto ch) {
        ...
        const size_t bcd_bin_buffer_size =
                binary_digits_.size() +
                4 * static_cast<size_t>(std::ceil(binary_digits_.size() / 3.0));
        ...
        for (size_t i{}; i < binary_digits_.size(); ++i) {
            for (long j = static_cast<long>(bcd_bin_buffer_size) - 1; j >= 3;
                 j -= 4) {
                unsigned value = (bcd_digits[j] ? 1 : 0) + (bcd_digits[j - 1] ? 2 : 0) +
                                 (bcd_digits[j - 2] ? 4 : 0) +
                                 (bcd_digits[j - 3] ? 8 : 0);

                if (value >= 5U) {
                    value += 3U;
                    bcd_digits[j] = (value & 1U) == 1U;
                    bcd_digits[j - 1] = (value & 2U) == 2U;
                    bcd_digits[j - 2] = (value & 4U) == 4U;
                    bcd_digits[j - 3] = (value & 8U) == 8U;
                }
            }
        ...
        decimal_number_str_.clear();
        decimal_number_str_.reserve(number_of_bcd_digits +
                                    (is_negative_number_ ? 1 : 0));
        ...
        for (size_t i{}; i <= bcd_digits.size() - 4; i += 4) {
            const byte value =
                    (bcd_digits[i] ? 8_b : 0_b) + (bcd_digits[i + 1] ? 4_b : 0_b) +
                    (bcd_digits[i + 2] ? 2_b : 0_b) + (bcd_digits[i + 3] ? 1_b : 0_b);
        ...
        std::for_each(std::cbegin(decimal_number_str_) + first_,
                      std::cend(decimal_number_str_), [&](const auto ch) {
...
void big_integer::get_binary_digits_for_decimal_number() {
    ...
    const size_t number_of_bits_needed_for_encoding_decimal_digits =
            number_str.length() *
            number_base_number_of_bits_.at(number_base::decimal);

    binary_digits_.reserve(number_of_bits_needed_for_encoding_decimal_digits);
    binary_number_str_.reserve(number_of_bits_needed_for_encoding_decimal_digits +
                               2 + (is_negative_number_ ? 1 : 0));
...
void big_integer::get_octal_digits_for_decimal_number() {
    ...
    const size_t remainder{binary_digits_.size() %
                           number_of_bits_needed_for_one_octal_digit};
    if (remainder != 0)
    ...
    octal_number_str_.reserve(binary_digits_.size() /
                              number_of_bits_needed_for_one_octal_digit +
                              2 + (is_negative_number_ ? 1 : 0));
    ...
    for (size_t i{};
        ...
        const byte value = (binary_digits_[i] ? 4_b : 0_b) +
                           (binary_digits_[i + 1] ? 2_b : 0_b) +
...
void big_integer::get_hexadecimal_digits_for_decimal_number() {
    ...
    const size_t remainder{binary_digits_.size() %
                           number_of_bits_needed_for_one_hex_digit};
    if (remainder != 0)
        binary_digits_.insert(std::cbegin(binary_digits_),
    ...
    hexadecimal_number_str_.reserve(binary_digits_.size() /
                                    number_of_bits_needed_for_one_hex_digit +
                                    2 + (is_negative_number_ ? 1 : 0));
    ...
    for (size_t i{};
        ...
        const byte value = (binary_digits_[i] ? 8_b : 0_b) +
                           (binary_digits_[i + 1] ? 4_b : 0_b) +
                           (binary_digits_[i + 2] ? 2_b : 0_b) +
                           (binary_digits_[i + 3] ? 1_b : 0_b);
...
std::string big_integer::div_by_two(const std::string &number_str) {
    ...
    if (modified_number_str != "0" && modified_number_str.length() > 1U &&
        modified_number_str.front() == '0') {
