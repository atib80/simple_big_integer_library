...
uint64_t bit_xor(const uint64_t first, const uint64_t second) {
    ...
    while (mask != 0ULL) {
        ...
        result |= (((first & mask) != 0ULL && (second & mask) == 0ULL) ||
                   ((first & mask) == 0ULL && (second & mask) != 0ULL)) ? mask : 0U;
        mask >>= 1U;
    }
...
TEST_CASE("big_integer::operator std::string() const noexcept",
    ...
    for (size_t i{}; i < number_of_tests; ++i) {
        ...
        random_double_value_str.erase(std::cbegin(random_double_value_str) + random_double_value_str.find('.'),
...
TEST_CASE(
    ...
    const uint64_t upper_limit{1LL << 32};
    for (size_t i{}; i < number_of_tests; ++i) {
...
TEST_CASE("big_integer operator%(const big_integer&, const big_integer&)",
    ...
    for (size_t i{}; i < number_of_tests; ++i) {
        ...
        const uint64_t remainder{first_operand % second_operand};
...
TEST_CASE(
        "big_integer operator--(const) const",
        "Testing big_integer's operator--(int) const public method") {
    big_integer positive_one{big_integer::plus_one};
...
TEST_CASE(
    ...
    for (size_t i{}; i < number_of_tests; ++i) {
        ...
        REQUIRE(std::to_string(first_operand_shifted) == bi1_shifted.get_decimal_number());
        REQUIRE(std::to_string(second_operand_shifted) == bi2_shifted.get_decimal_number());
    }
}
...
TEST_CASE(
        "big_integer& operator<<=(const size_t count)",
        "Testing big_integer's operator<<=(const size_t) public method") {
    const uint64_t lower_limit{};
    const uint64_t upper_limit{(1ULL << 32U) - 1};
    for (size_t i{}; i < number_of_tests; ++i) {
...
TEST_CASE("big_integer operator|(const big_integer &, const big_integer &)",
    ...
    for (size_t i{}; i < number_of_tests; ++i) {
        ...
        const uint64_t first_operand_and_second_operand{first_operand | second_operand};
        const big_integer bi1_and_bi2{bi1 | bi2};

        REQUIRE(std::to_string(first_operand_and_second_operand) == bi1_and_bi2.get_decimal_number());
    }
}
